...
%   M:输入明文，16位16进制数，字符串格式
%   K:输入密钥，16位16进制数，字符串格式
%  ---------------------------------------------
...
for i=1:16
    ...
    R_E=R(E0);%E盒扩展
    R_Ki=xor(R_E,Ki(i,:));%与48bit子密钥模二加
    %8个S盒构成选择压缩运算S
    %S1
    B=R_Ki(1:6);
    x=B(1)*2+B(6)+1;
    ...
    C=[C1,C2,C3,C4,C5,C6,C7,C8];%8个S盒的结果合并起来
    R_P=C(P);%经过P盒置换
    %交换左右两半部分
    TEMP=L;
    L=R;
